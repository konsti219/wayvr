...
struct WatchIds {
    ...
    media_artist: wgui::layout::WidgetID,
...
fn init_live_state(panel: &mut GuiPanel<WatchState>, app: &mut AppState) -> anyhow::Result<()> {
    ...
    let ids = WatchIds {
        ...
        media_artist: panel.parser_state.get_widget_id("media_artist_label")?,
    ...
    let media_play_glyph = CustomGlyphData::from_assets(
        ...
        AssetPathRef::BuiltIn("watch/media-play.svg"),
    )
    .ok();
    let media_pause_glyph = CustomGlyphData::from_assets(
        &panel.layout.state.globals,
        AssetPathRef::BuiltIn("watch/media-pause.svg"),
    )
    .ok();
...
impl WatchLiveState {
    ...
    fn refresh_media(&mut self, common: &mut CallbackDataCommon, app: &AppState) {
        ...
        // Separate labels so the artist keeps the lines it needs and the title
        // is clipped to whatever remains of the four-line budget.
        let (title, artist) = match media {
            None => ("No media".to_string(), String::new()),
            Some(media) => match (&media.title, &media.artist) {
                (Some(title), Some(artist)) => (title.clone(), artist.clone()),
                (Some(title), None) => (title.clone(), String::new()),
                _ => ("Nothing playing".to_string(), String::new()),
            },
        };
        set_label(common, self.ids.media_title, &title);
        set_label(common, self.ids.media_artist, &artist);
